...
send_stats () {
    ...
    echo "erx.vnstat.eth0.$1:$conv|g" | netcat -u -w 2 graph.za3k.com 8125
}

# Counters
send_stats 'counters.rx.bytes' "$(cat /proc/net/dev | grep '^\s*eth0:' | awk '{print $2}')"
send_stats 'counters.rx.packets' "$(cat /proc/net/dev | grep '^\s*eth0:' | awk '{print $3}')"
send_stats 'counters.tx.bytes' "$(cat /proc/net/dev | grep '^\s*eth0:' | awk '{print $10}')"
send_stats 'counters.tx.packets' "$(cat /proc/net/dev | grep '^\s*eth0:' | awk '{print $11}')"
